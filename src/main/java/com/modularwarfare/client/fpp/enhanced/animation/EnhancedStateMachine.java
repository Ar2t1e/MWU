...
public class EnhancedStateMachine {
    ...
    public int setReloadAnimation = 0;
    ...
    public AnimationType getReloadAnimationType() {
        ...
        if (reloadType == ReloadType.Load) {
                ...
                if (type.acceptedAmmo != null) {
                    ...
                    if (reloadPhase == Phase.FIRST) {
                        ...
                        if (!ItemGun.hasNextShot(heldItemstStack) && ((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.POST_RELOAD_EMPTY)) {
                            ...
                            if (((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.POST_RELOAD_SECOND)){
                                if (setReloadAnimation >= 0 && setReloadAnimation <= 3){
                                    aniType = AnimationType.POST_RELOAD;
                                }else if (setReloadAnimation >= 4 && setReloadAnimation <= 6){
                                    aniType = AnimationType.POST_RELOAD_SECOND;
                                }
                            }else {
                                aniType = AnimationType.POST_RELOAD;
                            }
            ...
            if (reloadPhase == Phase.FIRST) {
                if (!ItemGun.hasNextShot(heldItemstStack) && ((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.UNLOAD_EMPTY)) {
                    aniType = AnimationType.UNLOAD_EMPTY;
                }else {
                    aniType = AnimationType.UNLOAD;
                }
            } else if (reloadPhase == Phase.POST) {
                if (!ItemGun.hasNextShot(heldItemstStack) && ((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.POST_UNLOAD_EMPTY)) {
                    aniType = AnimationType.POST_UNLOAD_EMPTY;
                }else {
                    aniType = AnimationType.POST_UNLOAD;
                }
            } else if (reloadPhase == Phase.PRE) {
                if (!ItemGun.hasNextShot(heldItemstStack) && ((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.PRE_UNLOAD_EMPTY)) {
                    aniType = AnimationType.PRE_UNLOAD_EMPTY;
                }else {
    ...
    public AnimationType getShootingAnimationType() {
        ...
        if (shootingPhase == Phase.FIRST) {
            if (!ItemGun.hasNextShot(heldItemstStack)
                    && ((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.FIRE_EMPTY)) {

                aniType = AnimationType.FIRE_EMPTY;
            }else {
                if (type.isAiming == true){
                    if (shotCount == 0){
                        aniType = AnimationType.FIRE;
                    }else if (shotCount == 1){
                        if (((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.FIRE_SECOND)){
                            aniType = AnimationType.FIRE_SECOND;
                        }else {
                            aniType = AnimationType.FIRE;
                        }
                    }else if (shotCount == 2){
                        if (((GunEnhancedRenderConfig)currentModel.config).animations.containsKey(AnimationType.FIRE_THIRD)){
                            aniType = AnimationType.FIRE_THIRD;
                        }else {
                            aniType = AnimationType.FIRE;
                        }
                    }
    ...
    public void onRenderTickUpdate(float partialTick) {
        ...
        if (item instanceof ItemGun) {
            ...
            if (reloading) {
                ...
                if (reloadPhase != lastReloadPhase && aniType!=null) {
                    switch (aniType) {
                        ...
                        case POST_RELOAD_EMPTY:
                            ...
                            break;
                        case POST_RELOAD_SECOND:
                            ModularWarfare.NETWORK.sendToServer(new PacketGunReloadSound(WeaponSoundType.PostReloadSecond));
                            break;
                        default:
                            break;
                    }
                ...
                ClientProxy.gunEnhancedRenderer.controller.RELOAD = progess.get();
                if (!reloading) {
    ...
    public boolean canSprint() {
        return !shooting && !reloading && ClientProxy.gunEnhancedRenderer.controller.ADS < 0.8f;
    }
