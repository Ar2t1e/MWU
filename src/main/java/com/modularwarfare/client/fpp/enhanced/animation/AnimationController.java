...
import static com.modularwarfare.client.fpp.basic.renderers.RenderParameters.triggerPullSwitch;
...
public class AnimationController {
    ...
    public static float GetAtachModeProgress() {
    ...
    public void updateAction() {
        ...
        if (DRAW < 1F) {
            ...
            if(!ItemGun.hasNextShot(player.getHeldItemMainhand())) {
                if(((GunEnhancedRenderConfig)config).animations.containsKey(AnimationType.MODE_CHANGE_EMPTY)) {
                    this.playback.action = AnimationType.MODE_CHANGE_EMPTY;
                }
            }
    ...
    public void updateTime() {
        ...
        switch (this.playback.action){
            ...
            case INSPECT:
                ...
                break;
            case INSPECT_EMPTY:
                this.playback.updateTime(INSPECT_EMPTY);
                break;
            case MODE_CHANGE:
    ...
    public boolean shouldRenderAmmo() {
        EnhancedStateMachine anim = ClientRenderHooks.getEnhancedAnimMachine(player);
        if(anim.reloading) {
            if(anim.getReloadAnimationType()==AnimationType.POST_UNLOAD || anim.getReloadAnimationType()==AnimationType.POST_UNLOAD_EMPTY) {
                return false;
            }
            return true;
        }if (ClientTickHandler.reloadEnhancedPrognosisAmmoRendering != null
