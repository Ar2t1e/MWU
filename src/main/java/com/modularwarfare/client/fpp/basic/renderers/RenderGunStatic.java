...
//                }
                ...
                if (shouldRenderFlash) {
                    if(Minecraft.getMinecraft().currentScreen instanceof WorkbenchGuns) {

                    }else {
                        if (item.getTagCompound().hasKey("gunfire")) {
                            float flash = item.getTagCompound().getFloat("gunfire");
                            if (flash > 0){
                                if (anim.muzzleFlashTime > -0.01) {
                                    GlStateManager.pushMatrix();
                                    {
