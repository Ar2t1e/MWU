...
import com.modularwarfare.common.entity.*;
...
import com.modularwarfare.common.network.PacketCustomEffect;
import com.modularwarfare.common.network.PacketEXPEffect;
...
public class ClientProxy extends CommonProxy {
    ...
    public void onModelRegistry(ModelRegistryEvent event) {
        ...
        for (IngredientItem ingredientItem : ModularWarfare.ingredientTypes.values()) {
            ModelLoader.setCustomModelResourceLocation(ingredientItem, 0, new ModelResourceLocation(ModularWarfare.MOD_ID + ":" + ingredientItem.type.internalName));
        }
    ...
    public void onShootAnimation(EntityPlayer player, String wepType, int fireTickDelay, float recoilPitch, float recoilYaw) {
        ...
        if (gunType != null) {
            if (gunType.animationType == WeaponAnimationType.BASIC) {
                ...
                ClientRenderHooks.getEnhancedAnimMachine(player).triggerShoot(controller, (ModelEnhancedGun) gunType.enhancedModel, gunType, fireTickDelay);
            }
    ...
    public void spawnParticleExplosionRocket(World world, double x, double y, double z) {
        if(!world.isRemote) {
            super.spawnParticleExplosionRocket(world, x, y, z);
            return;
        }

        ModularWarfare.NETWORK.sendToServer(new PacketEXPEffect(x, y, z));
    }

    public void spawnParticleSmoke(World world, double x, double y, double z) {
        if(!world.isRemote) {
            super.spawnParticleSmoke(world, x, y, z);
            return;
        }

        //ModularWarfare.NETWORK.sendToServer(new PacketCustomEffect(x, y, z));
    }

    public void spawnParticleLeavesMark(World world, double x, double y, double z) {
        if(!world.isRemote) {
            super.spawnParticleLeavesMark(world, x, y, z);
            return;
        }
