...
import com.modularwarfare.common.block.BlockGunWorkbench;
import com.modularwarfare.craft.recipe.craftWorkbench;
...
//Кто Это Читает Знай Ты Гей А Если Ты Против Моего Заявления То Ты Ещё И Гей Отрицала (Обернись На Зад Я Уже С Зади 🧔)
@Mod(modid = ModularWarfare.MOD_ID)
public class ModularWarfare {
    //Да я шучу не бойся я друг или нет?
    public static final String MOD_ID = "modularwarfare";
    public static final String MOD_NAME = "ModularWarfareUltimate";
    public static final String MOD_VERSION = "2.4.5f";
    public static final String MOD_PREFIX = TextFormatting.GRAY+"["+TextFormatting.RED+"ModularWarfareUltimate"+TextFormatting.GRAY+"]"+TextFormatting.GRAY;
    public static final BlockGunWorkbench YOUR_CUSTOM_BLOCK_INSTANCE = new BlockGunWorkbench();
    ...
    public static HashMap<String, IngredientItem> ingredientTypes = new HashMap<String, IngredientItem>();
    ...
    public static void loadContent() {
        ...
        for (File file : contentPacks) {
            if (!MODS_TABS.containsKey(file.getName())) {
                if (!file.getName().equals("recipes")) { // Добавляем это условие
    ...
    public void onInitialization(FMLInitializationEvent event) {
        ...
        if (event.getSide() == Side.CLIENT) {
            craftWorkbench.registerCraftingRecipesWorkbench();
        }
    ...
    public void registerItems(RegistryEvent.Register<Item> event) {
        for (File file : contentPacks) {
            ...
            for (IngredientItem ingredientItem : ingredientTypes.values()) {
                if (ingredientItem.type.contentPack.equals(file.getName())) {
                    event.getRegistry().register(ingredientItem);
                    tabOrder.add(ingredientItem);
                }
            }
