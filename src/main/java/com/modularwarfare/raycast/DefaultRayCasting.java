            ...
            if (!Double.isNaN(vec32.x) && !Double.isNaN(vec32.y) && !Double.isNaN(vec32.z)) {
                ...
                while (k1-- >= 0) {
                    ...
                    if (ModConfig.INSTANCE.shots.shot_break_glass) {
                        if (block1 instanceof BlockGlass || block1 instanceof BlockStainedGlassPane || block1 instanceof BlockPane  || block1 instanceof BlockStainedGlass) {
                            //проверка материла
                            if (iblockstate1.getMaterial() == Material.GLASS){
                                world.setBlockToAir(blockpos);
                                ModularWarfare.NETWORK.sendToServer((PacketBase)new PacketDestroyBlock(blockpos));
                    ...
                    if (block1 instanceof BlockDoor || block1 instanceof BlockLeaves || block1 instanceof BlockPane) {
                        continue;
                    }
